...
class TerraMind(nn.Module):
    ...
    def forward_mod_loss(
        ...
        Args:
            y (torch.Tensor): Decoder tokens. Shape (B, M, D) where M = num_patches * num_codebooks for multicodebook modalities.
            target_ids (torch.Tensor): Ground truth token IDs. Shape (B, M).
            decoder_mod_dict (dict): dictionary containing tensor information for each modality in the decoder.
        ...
        for mod, d in decoder_mod_dict.items():
            idx = self.modality_info[mod]["id"]
            num_codebooks = self.modality_info[mod].get("num_codebooks", 1)

            # Get tokens for this modality
            mod_mask = decoder_mod_mask == idx
            y_mod = y[mod_mask]  # Shape: (N, D) where N = num_valid_tokens
            target_ids_mod = target_ids[mod_mask]  # Shape: (N,)

            if y_mod.numel() == 0:
                # If there are no logits / targets, set mod_loss to 0
                mod_loss[mod] = torch.zeros(1, device=y.device)
            else:
                # Get logits
                logits = self.decoder_embeddings[mod].forward_logits(y_mod)  # (N, vocab_size)

                if num_codebooks > 1:
                    # Reshape to separate codebooks
                    # N = num_patches * num_codebooks
                    # Reshape: (N, vocab_size) -> (num_patches, num_codebooks, vocab_size)
                    num_patches = y_mod.shape[0] // num_codebooks

                    logits = logits.reshape(num_patches, num_codebooks, -1)
                    target_ids_mod = target_ids_mod.reshape(num_patches, num_codebooks)

                    # Compute loss per codebook
                    codebook_losses = []
                    for c in range(min(3, num_codebooks)):  # Print first 3 codebooks
                        loss_c = F.cross_entropy(
                            logits[:, c, :],  # (num_patches, vocab_size)
                            target_ids_mod[:, c].long(),  # (num_patches,)
                            reduction="mean"
                        )
                        codebook_losses.append(loss_c)

                    # Compute remaining codebooks without printing
                    for c in range(3, num_codebooks):
                        loss_c = F.cross_entropy(
                            logits[:, c, :],
                            target_ids_mod[:, c].long(),
                            reduction="mean"
                        )
                        codebook_losses.append(loss_c)
                    ...
                    loss = F.cross_entropy(
                        logits, target_ids_mod.long(), reduction="mean"
                    )
                mod_loss[mod] = loss
        ...
        for mod, d in decoder_mod_dict.items():
            ...
            if y_mod.numel() == 0:
                ...
                if num_codebooks > 1:
                    ...
                    # Average across codebooks
                    loss = torch.stack(codebook_losses).mean()
                    # Count total number of tokens (patches * codebooks)
                    mod_count[mod] = num_patches * num_codebooks
                else:
                    # Single codebook case (backward compatible)
                    loss = F.cross_entropy(
                        logits, target_ids_mod.long(), reduction="mean"
